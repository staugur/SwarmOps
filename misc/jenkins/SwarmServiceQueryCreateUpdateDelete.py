...
# taochengwei <staugur@saintic.com> @2016-09-14
...
if __name__ == "__main__":
    ...
    headers={"User-Agent": "Jenkins/SwarmServiceQueryCreateUpdate"}
    ...
    serviceflag= os.getenv("serviceflag")

    if action == "query":
        if serviceflag:
            query.update(core=True, name=serviceflag, convert=True)
        else:
            query.update(core=True, convert=True)
        serviceUrl = apiUrl + "/service?" + urllib.urlencode(query)
        res = requests.get(serviceUrl )
